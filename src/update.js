...
import { Altervalue } from './tempvalues'
export let Updating=(index)=>{

    let[pos,setPos]=useState(index.one)
    let [getdetials,setDetials]=useState(
        {"name":index.mentions.name,
        "place":index.mentions.place,
        "fathername":index.mentions.fathername,
        "mothername":index.mentions.mothername,
        "date":index.mentions.date,
        "HSCmark":index.mentions.HSCmark,
        "SSLCmark":index.mentions.SSLCmark,
        "degree":index.mentions.degree,
        "number":index.mentions.number,
    ...
    const register=()=>{
        alert("Form updated successfully"+JSON.stringify(getdetials))
        Altervalue(getdetials,pos)
    }
    const cancel=()=>{
        alert("NOT UPDATED")
    }
    return(
        ...
        <div className='mt-2 text-center'>
            <button className='btn btn-outline-info col-5' onClick={register}>UPDATE</button>
        </div>
        <div className='mt-2 text-center'>
